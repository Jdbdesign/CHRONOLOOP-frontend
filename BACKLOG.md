...
- **`TeamMemberRow.module.css` has dead CSS**: its `.menuBtn`/`.menuCell`/hover-reveal rules style a 3-dot menu that `TeamMemberRow.tsx` never renders (no `Dropdown`, no `button`, confirmed by reading the component — only `TeamMemberGrid.tsx` imports this module, and it doesn't add a menu either). Not fixed in this phase (out of scope — R.7 covers touch targets/hover-affordances/swipe/testing, not dead-code cleanup), but worth removing in a future pass, or wiring up an actual menu if one was intended.
