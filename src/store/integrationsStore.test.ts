...
describe('integrationsStore', () => {
  ...
  it('connectApp changes status to connected', async () => {
    await useIntegrationsStore.getState().connectApp('google-drive')
    const app = useIntegrationsStore.getState().apps.find((a) => a.id === 'google-drive')
    expect(app?.status).toBe('connected')
  })

  it('disconnectApp changes status to available', async () => {
    await useIntegrationsStore.getState().connectApp('google-drive')
    await useIntegrationsStore.getState().disconnectApp('google-drive')
    const app = useIntegrationsStore.getState().apps.find((a) => a.id === 'google-drive')
    expect(app?.status).toBe('available')
  })

  it('toggleWebhook flips active state', async () => {
    const before = useIntegrationsStore.getState().webhooks[2].active
    await useIntegrationsStore.getState().toggleWebhook(2)
    expect(useIntegrationsStore.getState().webhooks[2].active).toBe(!before)
  })

  it('removeWebhook removes by index', async () => {
    const countBefore = useIntegrationsStore.getState().webhooks.length
    await useIntegrationsStore.getState().removeWebhook(0)
    expect(useIntegrationsStore.getState().webhooks.length).toBe(countBefore - 1)
  })

  it('toggleSyncRule flips on state', async () => {
    const before = useIntegrationsStore.getState().syncRules[3].on
    await useIntegrationsStore.getState().toggleSyncRule(3)
    expect(useIntegrationsStore.getState().syncRules[3].on).toBe(!before)
  })

  it('addApiKey adds a new key to the list', async () => {
    const countBefore = useIntegrationsStore.getState().apiKeys.length
    await useIntegrationsStore.getState().addApiKey({ label: 'Test Key', scope: 'Read Only', expires: 'Dec 31', rateLimit: '100', ipWhitelist: '' })
